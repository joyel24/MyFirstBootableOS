all: bootloader/boot.asm kernel/entry.asm kernel/kernel.c
	...
	i386-elf-gcc -o make_temp/kernel.o kernel/kernel.c -nolibc -nostdlib
	i386-elf-ld -T linker.ld -o make_temp/kernel.bin make_temp/entry.o make_temp/kernel.o --oformat binary
